...
public class ResumeGuidePage extends BasePager {
    private String category;
    private Context context;

    public ResumeGuidePage(Context context, String category) {
        super();
    ...
    private void refreshMoreNewData() {
        if ("1".equals(current_page)) {
            refrshOnePageData();//下拉刷新当前页面是第一个页面
        } else {
    ...
    private void refrshOnePageData() {
        OkHttpClient okHttpClient = new OkHttpClient();
