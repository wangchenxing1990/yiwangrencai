...
public class ChatActivity extends BaseActivity implements FuncLayout.OnFuncKeyBoardListener, View.OnClickListener {
    ...
    private void initData() {
        ...
        listCov=JMessageClient.getConversationList();
        if (!TextUtils.isEmpty(mTargetId)) {
            ...
            Log.i("++++++++", "mConv:" + mConv + ",mTargetId:" + mTargetId + ",mTargetAppKey:" + mTargetAppKey);
            if (mConv == null) {
                //先注册
                //注册成功之后
                ...
                EventBus.getDefault().post(new Event.Builder()
                        ...
                        .build());
                TextContent content = new TextContent("您好，我对贵公司的"+job_title+"职位很感兴趣，能否进一步沟通了解");
                    JMessageClient.sendMessage(mConv.createSendMessage(content));
                    submitFirstSendmessage();//提交第一次发送消息
            }
...
//        mChatAdapter.initMediaPlayer();
    ...
    private void submitFirstSendmessage() {
        ...
        form.add("resume_id",resume_id);
        form.add("com_id",com_id);
        form.add("job_id",job_id);
        form.add("type","1");
        Log.i("type","type");
        Request request=new Request.Builder()
                .url(ContentUrl.BASE_URL+"chat/send_first_chat_sms")
...
//                        ChooseAtMemberActivity.show(ChatActivity.this, ekBar.getEtChat(), mConv.getTargetId());
    ...
    public void startChatDetailActivity(String targetId, String appKey, long groupId) {
       // Log.i("mMyInfomMyInfo",listCov.size()+"listCov");
        Log.i("mMyInfomMyInfo",com_id+"listCov");
